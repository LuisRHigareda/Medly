...
def _section_header(pdf: FPDF, title: str) -> None:
    ...
    pdf.cell(CONTENT_WIDTH, 8, title, fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def build_prescription_pdf(record: MedicalRecord) -> bytes:
    ...
    pdf.cell(0, 10, "Medical Prescription", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)

    pdf.set_font("Helvetica", size=9)
    pdf.set_text_color(100, 100, 100)
    consultation_date = record.created_at.strftime("%B %d, %Y") if record.created_at else "N/A"
    pdf.cell(0, 6, f"Consultation date: {consultation_date}", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if record.procedures:
        for procedure in record.procedures:
            ...
            if procedure.notes:
                ...
                pdf.set_x(MARGIN + 10)  # deeper indent from left margin
    ...
    pdf.cell(0, 5, "Generated by Medly - MedicalRecordService", align="C", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
